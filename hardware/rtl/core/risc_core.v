...
// Implements Barrel Processor (Interleaved Multithreading) with 4 threads.
`timescale 1ns / 1ps
...
module risc_core #(
    ...
    // --- Signal Declarations ---
    wire [31:0] curr_pc[0:3], next_pc[0:3], pc_plus_4;
    wire [31:0] raw_instruction, fetch_pc_pass;
    wire        fetch_stall, hazard_stall, mem_stall;
    wire        pipe_en_all;
    wire        branch_taken;
    wire [31:0] branch_target;
    wire [1:0]  fetch_core_id_out;

    // Decode stage signals
    wire [31:0] id_instruction_in, id_pc_plus_4_in;
    wire [31:0] id_pc_in[0:3];
    wire [1:0]  id_core_id_in;
    wire [31:0] id_pc_plus_4_out, id_read_data1_out, id_read_data2_out, id_immediate_out;
    wire [31:0] id_pc_out[0:3];
    ...
    wire [1:0]  id_core_id_out;

    // Execute stage signals
    wire [31:0] ex_pc_plus_4_in, ex_read_data1_in, ex_read_data2_in, ex_immediate_in, ex_instruction_in;
    wire [31:0] ex_pc_in[0:3];
    wire [1:0]  ex_core_id_in;
    ...
    // Only update the PC of the current thread
    generate
        genvar i;
        for (i = 0; i < 4; i = i + 1) begin : pc_gen
            assign next_pc[i] = (branch_taken && (ex_core_id_out == i)) ? branch_target :
                                (pipe_en_all && (core_id == i))         ? pc_plus_4 :
                                curr_pc[i];
        end
    endgenerate

    ins_fetch fetch_stage (
        .clk(clk), .rst(rst), .core_id_in(core_id),
        .pipeline_stall_in(hazard_stall || mem_stall),
        .fetch_stall_out(fetch_stall),
        .pc_in(curr_pc[core_id]), .pc_plus_4_out(pc_plus_4),
    ...
    if_id_buffer if_id (
        .clk(clk), .rst(rst || (branch_taken && (ex_core_id_out == fetch_core_id_out))),
        .pipeline_stall(hazard_stall || mem_stall), .fetch_stall(fetch_stall),
        .core_id_in(fetch_core_id_out),
        ...
        .id_core_id_out(id_core_id_in)
    );

    ins_decode decode_stage (
        .clk(clk), .rst(rst),
        .id_core_id_in(id_core_id_in),
        ...
        .wb_core_id_in(wb_core_id_final),
        .pipeline_stall_out(hazard_stall),
        .id_pc_plus_4_out(id_pc_plus_4_out), .id_pc_out(id_pc_out), .id_read_data1_out(id_read_data1_out), .id_read_data2_out(id_read_data2_out),
        ...
        .id_core_id_out(id_core_id_out)
    );

    id_ex_buffer id_ex (
        .clk(clk), .rst(rst), .en(!mem_stall), .clr(branch_taken && (ex_core_id_out == id_core_id_out)),
        .id_core_id_in(id_core_id_out),
        .id_pc_plus_4_in(id_pc_plus_4_out), .id_pc_in(id_pc_out), .id_read_data1_in(id_read_data1_out), .id_read_data2_in(id_read_data2_out),
        ...
        .ex_core_id_out(ex_core_id_in)
    );

    ins_ex ex_stage (
        .id_core_id_in(ex_core_id_in),
        .id_pc_plus_4_in(ex_pc_plus_4_in), .id_pc_in(ex_pc_in[ex_core_id_in]), .id_read_data1_in(ex_read_data1_in), .id_read_data2_in(ex_read_data2_in),
    ...
    ex_ma_buffer ex_ma (
        ...
        .ma_core_id_out(ma_core_id_out)
    );

    ins_mem mem_stage (
        .clk(clk), .rst(rst),
        .ex_core_id_in(ma_core_id_out),
        ...
        .ma_core_id_out(wb_core_id_in)
    );

    mem_wb_buffer mem_wb (
        .clk(clk), .rst(rst), .en(1'b1),
        .ma_core_id_in(wb_core_id_in),
        ...
        .wb_core_id_out(wb_core_id_final)
    );

    ins_wb wb_stage (
        .clk(clk), .rst(rst),
        .ma_core_id_in(wb_core_id_final),
    ...
    always @(posedge clk) begin
        if (!rst && pipe_en_all) begin
            $display("T=%0t | Thread=%0d | PC=0x%h | Instr=0x%h", $time, id_core_id_in, id_pc_in[id_core_id_in], id_instruction_in);
        end
    end
