...
- **I UART RX: DONE** — `uart.v` is full duplex: 2FF-synced deserializer (mid-start-bit glitch rejection, LSB-first, stop not enforced) feeding a **16-byte RX FIFO**, registers at +0x04 STATUS (bit1 rx-ready, bit2 sticky overrun) and +0x08 RXDATA (read pops + clears overrun). Occupancy is an explicit 5-bit counter — 4-bit pointers can't distinguish full from empty at depth 16. Firmware: `uart_getchar()`/`uart_status()` in `drivers/uart.c`; echo app `apps/uart_echo.c`. TBs: `tb_uart_rx` (unit, drives the `uart` module directly via a new DUT mapping) and `tb_soc_uart_rx` (echo round trip through soc_top: single byte, back-to-back pair, 0x00–0xFF sweep exact, 64-byte burst → in-order prefix + recovery, glitch immunity).
  Full roadmap: `docs/plan_rtos_to_terminal.md`.
