...
- **H FreeRTOS: DONE (scheduler up)** — FreeRTOS V10.5.1 (vendored under `firmware/third_party/FreeRTOS`, M-mode CLINT port) boots on the SoC: `tb_soc_rtos` PASS — task A (prio 2) ticks 1-4 at exactly 25.05M cycles apart (500ms @ 50MHz), task B (prio 1) prints at 60.09M (1200ms), preemption correct. The bring-up exposed a real core bug (`load;load;branch` mis-evaluated because `decode_stage` gave the hazard `stall` priority over the pipeline-freeze `hold`, erasing a frozen load's `mem_read`) — fixed, full regression green; see `docs/rtos_bringup.md` + `tests/rtos/load_branch_bug.md`. **Queues DONE:** `tb_soc_rtos_q` PASS — producer/consumer tasks, 1000 items through a depth-8 `xQueue` in strict order (2.46M cycles); app `firmware/apps/rtos_q.c`, firmware Makefile links FreeRTOS for both `rtos` and `rtos_q`. Remaining: ISR-semaphore phase (needs machine-software interrupt in trap_unit), scheduler metric numbers (context-switch cost, tick jitter, ISR latency). Full roadmap: `docs/plan_rtos_to_terminal.md`.
