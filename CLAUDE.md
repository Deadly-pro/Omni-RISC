...
**Scope guard**: OoO execution (ROB, rename, scoreboard, dual-issue, branch prediction) is explicitly OUT of scope — archived on the `ooo-scope-archive` branch. Do not reintroduce it or suggest it. Build order: CPU → SoC/UART → caches → GPU → synthesis. See `docs/architecture.md`.
